...
describe("SettingsManager", () => {
    ...
    describe("escalating defaultMode trust filter", () => {
        ...
        it.each(["bypassPermissions", "auto", "acceptEdits"])("drops %s when set in project-tier settings", async (mode) => {
            const claudeDir = path.join(tempDir, ".claude");
            await fs.promises.mkdir(claudeDir, { recursive: true });
            await fs.promises.writeFile(path.join(claudeDir, "settings.json"), JSON.stringify({ permissions: { defaultMode: mode } }));
            settingsManager = new SettingsManager(tempDir);
            await settingsManager.initialize();
            expect(settingsManager.getSettings().permissions?.defaultMode).toBeUndefined();
        });
        it.each(["plan", "dontAsk"])("preserves non-escalating %s from project-tier settings", async (mode) => {
            const claudeDir = path.join(tempDir, ".claude");
            await fs.promises.mkdir(claudeDir, { recursive: true });
            await fs.promises.writeFile(path.join(claudeDir, "settings.json"), JSON.stringify({ permissions: { defaultMode: mode } }));
            settingsManager = new SettingsManager(tempDir);
        ...
        it("preserves escalating defaultMode when it comes from local-tier settings", async () => {
            ...
            const claudeDir = path.join(tempDir, ".claude");
            await fs.promises.mkdir(claudeDir, { recursive: true });
            await fs.promises.writeFile(path.join(claudeDir, "settings.local.json"), JSON.stringify({ permissions: { defaultMode: "acceptEdits" } }));
            settingsManager = new SettingsManager(tempDir);
            await settingsManager.initialize();
